...
| 3. Асинхронная архитектура с промежуточной очередью | Введение сервиса «Депозитный шлюз» + очередь на базе БД (до миграции на Kafka) | Соответствует +R-01/04, отказоустойчивость, подготовка к микросервисам | Требует разработки нового компонента, асинхронность усложняет UX                    | ✅ Рекомендуется |
